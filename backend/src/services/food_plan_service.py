...
# CREATE
async def cadastrar_food_plan(plan: FoodPlanCreate, user_id: str, db: Session):
    """
    ...
    """
    novo_plan = FoodPlan(**plan.model_dump(exclude_unset=True), user_id=user_id)
    db.add(novo_plan)
    db.commit()
...
# READ — listar todos
async def listar_food_plans(user_id: str, db: Session):
    """
    ...
    """
    return db.query(FoodPlan).filter(FoodPlan.user_id == user_id).all()


# READ — buscar por ID
async def buscar_food_plan(id: int, user_id: str, db: Session):
    """
    ...
    """
    return db.query(FoodPlan).filter(FoodPlan.id == id, FoodPlan.user_id == user_id).first()


# READ — buscar por paciente_id
async def buscar_food_plans_por_paciente(paciente_id: int, user_id: str, db: Session):
    """
    ...
    """
    return db.query(FoodPlan).filter(FoodPlan.paciente_id == paciente_id, FoodPlan.user_id == user_id).all()


# UPDATE
async def atualizar_food_plan(id: int, dados: FoodPlanCreate, user_id: str, db: Session):
    """
    ...
    """
    plano = db.query(FoodPlan).filter(FoodPlan.id == id, FoodPlan.user_id == user_id).first()
    if not plano:
        return None
...
# DELETE
async def deletar_food_plan(id: int, user_id: str, db: Session):
    """
